...
**Question 1.** \
Find the weather forecast for the next three days online. Create a list of lists in which each inner list should be labelled with the name of the day of the week. Each inner list should contain named elements telling you the temperature, chance of rain and wind speed.  temperature, chance of rain and wind speed

```{r}
weather_forecast <- list(
  "Thursday" = list(temperature = 3, chance_of_rain = "High", wind_speed = "6MPH"),
  "Friday" = list(temperature = 9, chance_of_rain = "low", wind_speed = "8mph"),
  "Saturday" = list(temperature = 5, chance_of_rain = "High", wind_speed = "13mph")
)
```
...
```{r}
weather_forecast <- c(6, 6, 9, 11, 8, 7,10,11,12,10,9,9,12,11)
two_weeks_weather <- matrix(weather_forecast,nrow = 2, ncol = 7, byrow = TRUE)
```
...
```{r}
two_weeks_weather[2,3]
two_weeks_weather[2,]
num_rows <- dim(two_weeks_weather)[1]
num_cols <- dim(two_weeks_weather)[2]
for (row_index in 1:num_rows) {
  for (col_index in c(1,3,5)) {
    print(two_weeks_weather[row_index,col_index])
