...
```{r}
 mobile_data %>%
  ...
  stat_function(
    fun = dnorm, 
    args = list(
      mean = top_phone_rating$mean, 
      sd = top_phone_rating$sd
    ),
    col = "red"
  )
```
...
```{r}
mobile_data %>%
  filter(rating >= top_phone_rating$mean - top_phone_rating$sd) %>%
  filter(rating <= top_phone_rating$mean + top_phone_rating$sd) %>%
  summarise(prop_within_1sd = n() / nrow(mobile_data))
```
